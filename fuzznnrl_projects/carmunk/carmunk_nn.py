...
def neural_net(num_inputs, params, lr=0.001, loss="mean_squared_error", load='', use_dropout=True,
               drop_prob=0.2):
    # create model
    model = Sequential()
    ...
    # hidden layer
    model.add(Dense(units=params[4], kernel_initializer='he_uniform'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    if use_dropout:
        model.add(Dropout(drop_prob))
    ...
    # output layer
    model.add(Dense(units=params[5], kernel_initializer='he_uniform'))
    model.add(BatchNormalization())
